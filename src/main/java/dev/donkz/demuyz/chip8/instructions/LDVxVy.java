...
public class LDVxVy extends Instruction {
    public LDVxVy(int instructionCode, Chip8 cpu) {
        super(instructionCode, cpu, 2);
    }

    @Override
    public void execute() {
        logger.debug("LD Vx, Vy", "Instruction");

        Assertions.assertRegister(parameters[0]);
        Assertions.assertRegister(parameters[1]);

        cpu.getRegister(parameters[0]).setValue(cpu.getRegister(parameters[1]).getValue());
    }
